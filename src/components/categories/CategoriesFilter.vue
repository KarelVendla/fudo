<template>
    <div class="d-flex flex-row">
        <Multiselect style="margin-right: 10px;" @input="updateFilters({ category_code: $event ? $event.code : $event })" v-model="category" select-label="Filter" deselect-label="Remove" track-by="name" label="name" placeholder="Filter by category" :options="categories" :allow-empty="true">
